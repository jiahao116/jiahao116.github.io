...
import type { EntryItem } from '@/components/home/EntrySection';
...
interface SectionConfig {
  id: string;
  type: 'markdown' | 'publications' | 'list' | 'entries';
  title?: string;
  source?: string;
  filter?: string;
  limit?: number;
...
function processSections(sections: SectionConfig[], locale?: string): SectionConfig[] {
  return sections.map((section: SectionConfig) => {
    switch (section.type) {
      ...
      case 'entries': {
        const sourceData = section.source
          ? getTomlContent<Record<string, { intro?: string; entries?: EntryItem[] }>>(section.source, locale)
          : null;
        const collection = section.collection ? sourceData?.[section.collection] : undefined;
        return {
          ...section,
